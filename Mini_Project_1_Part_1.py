...
def plot_piechart_helper(sorted_cosine_scores_items):
    #print("CHECKKK", sorted_cosine_scores_items[][1])
...
def cosine_similarity(x, y):
    ...
    # 2. Exponentiate cosine similarity
    exponentiated_similarity = np.exp(cosine_similarity)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type=50):
    ...
    for word in words:
        if word in word_index_dict:
            # 2. Get embeddings for each word
            word_embedding = embeddings[word_index_dict[word]]
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        if input_embedding is None or len(input_embedding) == 0:
            print("ERROR: GloVe input embedding is empty!")  


        # Get category embeddings
        category_embeddings = {}
        for category in categories:
            print("DURINGGG GLOVEE", category)
            category_embeddings[category] = averaged_glove_embeddings_gdrive(
                category, word_index_dict, embeddings, model_type
            )
            cosine_sim[category] = 1 - cosine_similarity(input_embedding, category_embeddings[category])
        ...
        # Compute category embeddings and similarity
        for category in categories:
            print("DEBUG: Processing category ->", category)
            
            if category not in category_embeddings:
                # If category embedding is missing, compute and store it
                if model_name:
            ...
            # Compute cosine similarity between input sentence and category
            cosine_sim[category] = 1 - cosine_similarity(input_embedding, category_embeddings[category])
        #print("DEBUG: CATROGORY EMBEDDINGGG", category_embeddings[category])

            

    # Sort cosine similarity (higher similarity first)
